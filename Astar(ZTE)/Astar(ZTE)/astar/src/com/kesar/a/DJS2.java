 ...
 * 
 * ClassName: AStar 
 * @Description: 用来进行最后与A*算法计算比较最优路径的djs算法（包括绘制路径函数） *计算DJS的距离，并打印路径
 * @author njust
 */
public class DJS2
{
	public final static int BAR = 1; //     0是可走的，1表示障碍物不可走
	public final static int PATH =9; //    2是路线
	public final static int max = -1; //    2是路线
	public int number=0;
	Queue<Node> openList = new PriorityQueue<Node>(); // 优先队列(升序)
	List<Node> closeList = new ArrayList<Node>();
	
	//開始算法，循环移动结点寻找路径，设定循环结束条件。Open表为空或者终于结点在Close表 b[0]为搜索节点个数， b[1]为代价
	...
	{int[] b={0,0};
		...
		b[1]=dis(mapInfo.maps, mapInfo.end);
		//返回距离
		return b;
	}
	...
	private int moveNodes(MapInfo mapInfo)
	{
		while (!openList.isEmpty())
		{
			...
			{
				//System.out.println(" DJS算法遍历的节点个数"+number);
    ...
    * 增加一个邻结点到open表，，，，，，，，，，，，，，，，，，，，，，，，，，，，，，，，，，，，，，，改动距离在这里改
	...
	{   int cost=(int)Math.sqrt(Math.abs(coord.x-end .x)*Math.abs(coord.x-end .x)+Math.abs(coord.y-end .y)*Math.abs(coord.y-end .y))*15;
	    ;
		return 0;
	}
